...
export default function BookingModal({
  ...
  const basePrice = Number(tourPrice ?? priceFrom ?? 0);
  const totalBaseGBP = basePrice * guests;
    ...
    >
      ...
      >
        <button
          ...
          className="absolute right-5 top-5 text-black/40 hover:text-black p-1 rounded-full hover:bg-slate-100 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <form onSubmit={handleWhatsAppBooking} className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-black/5 my-2">
            <div>
              ...
              <span className="text-xs text-black/40">
                {formatPrice(basePrice)} × {guests} guest{guests > 1 ? "s" : ""}
              </span>
            </div>
          ...
          <button
            type="submit"
            className="w-full bg-[#25D366] hover:bg-[#20bd5a] text-white py-3 px-6 rounded-xl font-bold text-sm inline-flex items-center justify-center gap-2 shadow-sm transition-all cursor-pointer"
          >
